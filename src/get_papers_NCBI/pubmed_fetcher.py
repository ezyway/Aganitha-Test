...
class PubMedFetcher:
    ...
    def __init__(self, query: str, api_key: str, max_results: int = 20) -> None:
        """
        ...
        """
        self.api_key: str = api_key               # API key for authentication and rate limits
        self.query: str = query                   # Search query to send to PubMed
        self.max_results: int = max_results       # Limit on number of results to fetch
        self.pmids: List[str] = []                # List to store PubMed IDs of matching papers
        self.papers_data: List[Dict[str, str]] = []  # Detailed information for each paper
        self.session: requests.Session = requests.Session()    # Reuse the session for efficient network calls

    def fetch_pmids(self) -> None:
        """
        ...
        """
        search_query: str = self.query       # PubMed search term
        retmax: int = 100                    # Number of results to fetch per request
        retstart: int = 0                    # Offset for pagination
        ...
        while len(self.pmids) < self.max_results * 5:
            ...
            try:
                ...
                response: requests.Response = self.session.get(f"{self.BASE_URL}esearch.fcgi", params=params)
                response.raise_for_status()  # Raise exception for bad responses

                # Parse XML response and extract PMIDs
                root: ET.Element = ET.fromstring(response.content)
                ids: List[str] = [id_elem.text for id_elem in root.findall('.//IdList/Id') if id_elem.text]
    ...
    def fetch_paper_details(self) -> None:
        """
        ...
        batch_size: int = 100  # Number of PMIDs to fetch in each batch

        # Process PMIDs in batches
        for i in range(0, len(self.pmids), batch_size):
            batch_pmids: List[str] = self.pmids[i:i + batch_size]  # Slice out a batch of PMIDs
    ...
    def _fetch_batch_details(self, pmids: List[str]) -> None:
        """
        ...
        try:
            ...
            response: requests.Response = self.session.get(f"{self.BASE_URL}efetch.fcgi", params=params)
            ...
            for article in root.findall('.//PubmedArticle'):
                ...
                paper_data: Optional[Dict[str, str]] = self._extract_paper_data(article)
    ...
    def _extract_paper_data(self, article: ET.Element) -> Optional[Dict[str, str]]:
        """
        ...
        try:
            # Extract PubMed ID
            pmid: str = article.findtext('.//PMID') or "Unknown"

            # Extract article title
            title: str = article.findtext('.//ArticleTitle') or "No title"

            # Extract publication date (delegated to helper method)
            pub_date: str = self._extract_publication_date(article)

            # Extract authors and their affiliations (returns a list of dicts)
            authors_info: List[Dict[str, Any]] = self._extract_authors_and_affiliations(article)

            # Identify authors with commercial/company affiliations
            company_authors: List[str] = self._filter_company_authors(authors_info)
            ...
            corresponding_email: str = self._extract_corresponding_email(article)

            # Extract company-related affiliations for those authors
            company_affiliations: List[str] = self._extract_company_affiliations(authors_info)
    ...
    def _extract_publication_date(self, article: ET.Element) -> str:
        ...
        """
        pub_date: Optional[ET.Element] = article.find('.//PubDate')  # Look for publication date node
        if pub_date is not None:
            # Extract year, month, and day if available
            year: Optional[str] = pub_date.findtext('Year')
            month: Optional[str] = pub_date.findtext('Month')
            day: Optional[str] = pub_date.findtext('Day')

            # Join available parts into a date string (e.g., "2023-May-15")
            date_parts: List[str] = [part for part in [year, month, day] if part]
            return '-'.join(date_parts) if date_parts else "Unknown"
        
        return "Unknown"  # Return fallback if date is missing

    def _extract_authors_and_affiliations(self, article: ET.Element) -> List[Dict[str, Any]]:
        """
        ...
        for author in article.findall('.//AuthorList/Author'):
            ...
            last: Optional[str] = author.findtext('LastName')
            fore: Optional[str] = author.findtext('ForeName')
            
            # Construct full name or fallback to last name / "Unknown"
            name: str = f"{fore} {last}" if fore and last else last or "Unknown"
    ...
    def _is_company_affiliation(self, affiliation: str) -> bool:
        ...
        """
        aff_lower: str = affiliation.lower()  # Normalize text for case-insensitive comparison
    ...
    def _filter_company_authors(self, authors_info: List[Dict[str, Any]]) -> List[str]:
        """
    ...
    def _extract_company_affiliations(self, authors_info: List[Dict[str, Any]]) -> List[str]:
        """
    ...
    def _extract_corresponding_email(self, article: ET.Element) -> str:
        ...
        text_fields: List[ET.Element] = article.findall('.//AbstractText')

        for elem in text_fields:
            if elem.text:
                # Use regex to locate the first email-like string
                match: Optional[re.Match[str]] = re.search(r'[\w\.-]+@[\w\.-]+', elem.text)
                if match:
